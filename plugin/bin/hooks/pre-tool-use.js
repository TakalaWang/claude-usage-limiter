...
import { mkdirSync as mkdirSync4, readFileSync as readFileSync5, writeFileSync as writeFileSync3 } from "node:fs";
...
function writeCache(cache) {
  try {
    mkdirSync3(dirname2(SCAN_CACHE_PATH), { recursive: true });
...
function throttled(now) {
  ...
  try {
    const raw = readFileSync5(THROTTLE_PATH, "utf8").trim();
    const t = Number(raw);
...
function stampThrottle(now) {
  lastCheckMs = now;
  try {
    mkdirSync4(dirname3(THROTTLE_PATH), { recursive: true });
    writeFileSync3(THROTTLE_PATH, String(now));
  } catch {
  }
}
function denyOutput(reason) {
  return {
    hookSpecificOutput: {
      ...
      permissionDecisionReason: `${reason} DO NOT retry this tool call \u2014 you are over the configured weekly budget. End the current turn so the user can adjust the limit or wait for the reset.`
    }
  };
}
async function main() {
